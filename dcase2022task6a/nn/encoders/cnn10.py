...
from dcase2022task6a.nn.functional.pann import init_bn, init_layer, do_mixup
from dcase2022task6a.nn.modules.conv_block import ConvBlock
...
class CNN10(nn.Module):
    ...
    def __init__(
        ...
        fmax: int = 14000,
        # Other args
        add_clip_linear: bool = True,
        add_frame_linear: bool = False,
        classes_num: int = 527,
        ...
        if pretrained and add_clip_linear and classes_num != self.AUDIOSET_CLASSES_NUM:
            logging.warning(
                f"Found argument {classes_num=} != {self.AUDIOSET_CLASSES_NUM} and {pretrained=}, so the layer 'fc_audioset' will not use pretrained weights."
            )
        if pretrained and add_clip_linear and clip_emb_size != 512:
            raise ValueError(
                f"Invalid argument {clip_emb_size=} with {pretrained=} and {add_clip_linear=}."
            )
        if not add_frame_linear and frame_emb_size != 512:
            raise ValueError(
                f"Cannot remove Linear 'fc2' in CNN10 with {add_frame_linear=} and {frame_emb_size=} != 512. Please use add_frame_linear=True or frame_emb_size=512."
            )
        ...
        super().__init__()
        # Params
        self.add_clip_linear = add_clip_linear
        self.add_frame_linear = add_frame_linear
        ...
        # For tag outputs
        self.fc1 = (
            nn.Linear(conv_features_emb_size, clip_emb_size, bias=True)
            if add_clip_linear
            else nn.Identity()
        )
        self.fc_audioset = (
        ...
        self.fc2 = (
            nn.Linear(conv_features_emb_size, frame_emb_size, bias=True)
            if add_frame_linear
            else nn.Identity()
        )
        ...
        if pretrained:
            ...
            self.load_pretrained_weights(strict=False, exclude_spectro_params=exclude_spectro_params)
    ...
    def load_pretrained_weights(
        ...
        fpath = osp.join(torch.hub.get_dir(), "checkpoints", "Cnn10_mAP=0.380.pth")
        ...
        if self.pretrained and self.classes_num != self.AUDIOSET_CLASSES_NUM:
            model_state_dict = {
                key: weight
                for key, weight in model_state_dict.items()
