...
class LogLRCallback(Callback):
    """Log the learning rate (lr) in the logger and each iteration."""
        ...
        **kwargs,
        ...
        for i, optimizer in enumerate(optimizers):
            ...
            for j, lr in enumerate(learning_rates):
                ...
                pl_module.log(
                    name, lr, on_epoch=False, on_step=True, batch_size=self.bsize
                )
    ...
    def on_train_batch_end(
        ...
        **kwargs,
        ...
        for i, (count, threshold) in enumerate(zip(counts, thresholds)):
            ...
            pl_module.log(
                name, prop, on_epoch=False, on_step=True, batch_size=self.bsize
            )


class LogGradNorm(Callback):
    def __init__(
        self, prefix: str = "train/", p_norm: int = 2, bsize: Optional[int] = None
    ) -> None:
        super().__init__()
        self.prefix = prefix
        self.p_norm = p_norm
    ...
    def on_train_batch_end(
        ...
        **kwargs,
        ...
        pl_module.log(
            f"{self.prefix}grad_norm{self.p_norm}",
            grad_norm,
            on_epoch=True,
            on_step=True,
